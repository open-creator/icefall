...
path = "data/fbank/gigaspeech_cuts_M-standard_fbank.jsonl.gz"
disc_path = "data/fbank/gigaspeech_cuts_M.jsonl.gz"
transform_path = "data/fbank/gigaspeech_cuts_M_fbank_disc.jsonl"
...
# 打开gzip文件
updated_data_list = []
with gzip.open(disc_path, 'rt', encoding='utf-8') as file:
    # 遍历文件中的每一行
    previous_data = None
    for line in file:
        ...
        if previous_data is not None:
            ...
            if previous_dialog == current_dialog:
                current_no_change_data = copy.deepcopy(data_list[0])
                current_data['custom']['previous_data'] = copy.deepcopy(previous_data)
                previous_data = current_no_change_data
                data_list = data_list[1:] if len(data_list) > 1 else data_list
                
            else:
                current_no_change_data = copy.deepcopy(data_list[0])
                current_data['custom']['previous_data'] = copy.deepcopy(data_list[0])
                previous_data = current_no_change_data
                data_list = data_list[1:] if len(data_list) > 1 else data_list
        else:
            previous_data = copy.deepcopy(data_list[0])

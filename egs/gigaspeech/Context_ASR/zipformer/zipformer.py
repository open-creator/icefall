...
class Zipformer2(EncoderInterface):
    ...
    def __init__(
        ...
        if context_emb:
            self.history_emb = torch.nn.Parameter(torch.zeros(1,encoder_dim[0]))  
            self.current_emb = torch.nn.Parameter(torch.zeros(1,encoder_dim[0]))
            self.future_emb = torch.nn.Parameter(torch.zeros(1,encoder_dim[0]))
        ...
        for i in range(num_encoders):
            ...
            encoder = Zipformer2Encoder(
                ...
                history_emb=self.history_emb if context_emb else None,
                current_emb=self.current_emb if context_emb else None,
                future_emb=self.future_emb if context_emb else None,
    ...
    def forward(
        ...
        previous_outputs: Optional[List[Tensor]] = None,
        future_outputs: Optional[List[Tensor]] = None,
    ) -> Tuple[Tensor, Tensor]:
        """
        Args:
          x:
        ...
        # step 1
        history_tensor = torch.tensor(1) * self.history_emb
        current_tensor = torch.tensor(1) * self.current_emb
        future_tensor = torch.tensor(1) * self.future_emb
        for i, module in enumerate(self.encoders):
            ds = self.downsampling_factor[i]
            x = convert_num_channels(x, self.encoder_dim[i])
            if self.context_emb:
                history_tensor = convert_num_channels(history_tensor, self.encoder_dim[i])
                current_tensor = convert_num_channels(current_tensor, self.encoder_dim[i])
                future_tensor = convert_num_channels(future_tensor, self.encoder_dim[i])
            ...
            x = module(x,
                       ...
                       attn_mask=attn_mask,
                       previous_outputs=previous_outputs[i] if previous_outputs is not None else None,
                       future_outputs=future_outputs[i] if future_outputs is not None else None,
                       history_tensor=history_tensor if self.context_emb else None,
                       current_tensor=current_tensor if self.context_emb else None,
                       future_tensor=future_tensor if self.context_emb else None,
                       )
            outputs.append(x)
...
class Zipformer2EncoderLayer(nn.Module):
    ...
    def __init__(
            ...
            history_emb: torch.Tensor = None,
            current_emb: torch.Tensor = None,
            future_emb: torch.Tensor = None,
    ) -> None:
        super(Zipformer2EncoderLayer, self).__init__()
        self.embed_dim = embed_dim
        self.history_emb = history_emb 
        self.current_emb = current_emb
        self.future_emb = future_emb
    ...
    src_key_padding_mask:  the mask for padding, of shape (batch_size, seq_len); True means
        ...
        na = self.balancer_na(self.nonlin_attention(src, selected_attn_weights, previous_outputs, future_outputs))

        src = src + (na if self_attn_dropout_mask is None else na * self_attn_dropout_mask)

        self_attn = self.self_attn1(src, attn_weights, previous_outputs, future_outputs)
        ...
        self_attn = self.self_attn2(src, attn_weights, previous_outputs, future_outputs)
...
class Zipformer2Encoder(nn.Module):
    ...
    def forward(
        ...
        #     pos_emb = self.encoder_pos(torch.cat[previous_outputs, src], 0)
        # else:
        pos_emb = self.encoder_pos(src)
        output = src

        # get previous mask
        previous_emb = self.encoder_pos(previous_outputs) if previous_outputs is not None else None
        pos_emb = torch.cat([previous_emb, pos_emb], dim=1) if previous_emb is not None else pos_emb

        future_emb = self.encoder_pos(future_outputs) if future_outputs is not None else None
        pos_emb = torch.cat([pos_emb, future_emb], dim=1) if future_emb is not None else pos_emb

        if history_tensor is not None and previous_outputs is not None:
            history_tensor = history_tensor.expand_as(previous_outputs)
            current_tensor = current_tensor.expand_as(src)
            output = output + current_tensor 
            previous_outputs = previous_outputs + history_tensor 
            future_tensor = future_tensor.expand_as(future_outputs) if future_outputs is not None else None
            future_outputs = future_outputs + future_tensor if future_tensor is not None else None
...
class DownsampledZipformer2Encoder(nn.Module):
    ...
    def forward(
        ...
        previous_outputs = self.downsample(previous_outputs) if previous_outputs is not None else None
...
class RelPositionMultiheadAttentionWeights(nn.Module):
    ...
    def forward(
        ...
        # step 4
        x = self.in_proj(x)
        previous_outputs = self.in_proj(previous_outputs) if previous_outputs is not None else None
        future_outputs = self.in_proj(future_outputs) if future_outputs is not None else None
        query_head_dim = self.query_head_dim
        pos_head_dim = self.pos_head_dim
        num_heads = self.num_heads

        seq_len, batch_size, _ = x.shape
        total_len = seq_len + (0 if previous_outputs is None else previous_outputs.shape[0])
        p_len, _, _ = previous_outputs.shape if previous_outputs is not None else (0, 0, 0)

        total_len = total_len + (0 if future_outputs is None else future_outputs.shape[0])
        f_len, _, _ = future_outputs.shape if future_outputs is not None else (0, 0, 0)
        ...
        k = torch.cat([previous_outputs[...,query_dim:2*query_dim], k], 0) if previous_outputs is not None else k

        p = torch.cat([previous_outputs[..., 2*query_dim:], p], 0) if previous_outputs is not None else p

        k = torch.cat([future_outputs[...,query_dim:2*query_dim], k], 0) if future_outputs is not None else k
        p = torch.cat([future_outputs[...,2*query_dim:], p], 0) if future_outputs is not None else p
        ...
        if p_len > 0:
            if f_len > 0:
                p = p.reshape(seq_len + p_len + f_len, batch_size, num_heads, pos_head_dim) 
                k = k.reshape(seq_len + p_len + f_len, batch_size, num_heads, query_head_dim)
            else:
                p = p.reshape(seq_len + p_len, batch_size, num_heads, pos_head_dim) 
                k = k.reshape(seq_len + p_len, batch_size, num_heads, query_head_dim)
        else:
        ...
        # k (num_heads, batch_size, seg_len, total_len) 
        # attn_scores (num_heads, batch_size, seq_len, total_len)
        ...
        if use_pos_scores:
            ...
            if previous_outputs is None:
                ...
                pos_emb = pos_emb[:, -total_len:, :].reshape(-1, total_len, num_heads, pos_head_dim).permute(2, 0, 3, 1)
            ...
            if torch.jit.is_tracing():
                ...
                if previous_outputs is None:
                    ...
                    pos_scores = pos_scores.as_strided((num_heads, batch_size, seq_len, total_len),
                                                    (pos_scores.stride(0),
                                                        pos_scores.stride(1),
                                                        pos_scores.stride(2)-pos_scores.stride(3),
                                                        pos_scores.stride(3)),
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            ...
            attn_scores = penalize_abs_values_gt(attn_scores,
                                                 ...
                                                 penalty=1.0e-04,
                                                 name=self.name)
        #attn_scores = attn_scores[..., : (attn_scores.size(-1) - f_len)] if future_outputs is not None else attn_scores

        #attn_scores = attn_scores[..., -seq_len:] if previous_outputs is not None else attn_scores
        attn_scores = attn_scores / math.sqrt(query_head_dim) if previous_outputs is not None else attn_scores
        ...
        if key_padding_mask is not None:
            ...
            previous_key_padding_mask = torch.zeros((batch_size, total_len - seq_len), device=key_padding_mask.device, dtype=torch.bool) if previous_outputs is not None else None
            key_padding_mask = torch.cat([previous_key_padding_mask, key_padding_mask], dim=1) if previous_outputs is not None else key_padding_mask
    ...
    def forward(
        ...
        if previous_outputs is not None:
            if future_outputs is not None:
                x = torch.cat([future_outputs, x], 0)
            x = torch.cat([previous_outputs, x], 0)
...
attn_weights: a Tensor of shape (num_heads, batch_size, seq_len, seq_len)
        ...
        if previous_outputs is not None:
            ...
            if future_outputs is not None:
                x = torch.cat([future_outputs, x], 0)
            x_p = torch.cat([previous_outputs, x], 0)
            x = x_p
            x = self.in_proj(x)
            s, x, _ = x.chunk(3, dim=-1)
